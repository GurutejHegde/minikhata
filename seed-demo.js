...
async function seed() {
  ...
  try {
    ...
    const [userResult] = await conn.query(
      `INSERT INTO users (username, password, user_type, created_at) VALUES (?, ?, 'business', '2025-01-10')`,
      ['gurutej', hashedPassword]
    );
    const userId = userResult.insertId;

    // ─── 3. INSERT 6 CUSTOMERS ────────────────────────────────────────────
    console.log('👥 Adding 6 customers...');
    const customers = [
    ...
    console.log('⚙️  Running FIFO settlement engine...');
